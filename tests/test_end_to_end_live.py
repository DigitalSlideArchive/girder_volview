...
def _stack_up(timeout=2.0):
    ...
    try:
        with urllib.request.urlopen(
            API_ROOT + "/system/version", timeout=timeout
        ) as resp:
...
def _proxiable_uri(file_doc):
    # The backend's own mint (utils.makeFileDownloadUrl): origin-relative
    # /api/v1/file/<id>/proxiable/<name>. resolveInputUrisToFiles recovers the
...
def _assert_labelmap_result(gc, folder_id, job_id, final, tmp_path):
    ...
    # /results is the `{intents, missing}` envelope.
...
def e2e_folder(gc):
    ...
    folder = gc.createFolder(
        me["_id"],
        "volview-e2e-%s" % uuid.uuid4().hex[:8],
        parentType="user",
        public=False,
    )
    yield folder
...
def test_single_volume_result_correlates_to_job(gc, e2e_folder, tmp_path):
    ...
    # entries -- the backend autofills output names (_autofillOutputs).
    values = {
    ...
    _assert_labelmap_result(gc, e2e_folder["_id"], job_id, final, tmp_path)
